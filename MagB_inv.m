...
if any(isnan(matrix(:))); errm=3;contourf(x,y,matrix);return;end
else
...
[T,x,y,nx,ny,xmin,xmax,ymin,ymax,dx,dy,errm]=lodgrdatfile_Callback(sourcfil);
 if errm==1; errormess (1);return;end
 if errm==4; errormess (4);return;end
 if errm==3; errormess (3);return;end
 if errm==100; errormess (100);return;end
%%%%%%%set an initial value for sh and wh 
setto=get(findobj(gcf,'Tag','tabl1'),'Data');
sh_initial=0.1/dx;    
wh_initial=0.5*sh_initial;
...
try
T=0;x=0;y=0;nx=0;ny=0;xmin=0;xmax=0;ymin=0;ymax=0;dx=0;dy=0;errm=100;
data=load(sourcfil);
...
if nx*ny~=rowsn;errm=3;plot(x,y,'+');return;end;
errm=0;
if x(2)==x(1);
T=reshape(T,ny,nx);
x=reshape(x,ny,nx);
...
text(0.1,.5,'XYZ data input is not equal spaced..(check intervals of X or Y colums)','Color','w')
    case 100
text(0.1,.5,'Unknown format error...','Color','w')
end
axis off
end
